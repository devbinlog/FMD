...
| Provider Layer | 모든 프로바이더는 `BaseProvider.search(keywords, category, color, limit)` 구현. 팬아웃·수집은 검색 엔드포인트가 담당. Search Provider는 Naver Image Search → Google Custom Search → Openverse 순으로 시도. |
| Ranking Engine | `RankingWeights` 데이터클래스를 파라미터로 주입받아 가중치를 런타임에 교체 가능. 4개의 독립 신호로 채점, 신호별 점수 분해와 설명 목록 반환. |
...
단일 프로바이더는 재현율(recall)에 한계가 있습니다. 큐레이션된 Mock 샘플은 안정적인 기본 커버리지를 제공하고, API 프로바이더는 안정적인 메타데이터를 가진 라이선스 에셋을 공급하며, Search Provider는 Naver와 Google을 통해 실시간 검색 결과를 가져옵니다. 소스의 합집합이 재현율을 향상시키는 대신 지연 시간이 추가되는 트레이드오프가 있습니다 — 검색이 사용자의 주의를 이미 차지하는 분석 단계 이후에 실행되므로 허용 가능한 트레이드오프입니다.

Search Provider의 우선순위 체인: Naver Image Search(한국어 콘텐츠, 썸네일 풍부) → Google Custom Search(국제 결과) → Openverse fallback(API 키 불필요). API 키 없이도 Openverse로 동작합니다.
...
```python
# services/ranking.py — 실제 구현
@dataclass
class RankingWeights:
    w_embedding: float = 0.55   # 시맨틱 유사도 (TF-IDF 코사인)
    w_color:     float = 0.20   # 시각적 유사도 (RGB 유클리드 거리)
    w_keyword:   float = 0.20   # 태그 겹침 수
    w_meta:      float = 0.05   # 이미지 URL 존재 + 중복 제거
    # 임베딩 없을 때 폴백
...
def rank_results(..., weights: RankingWeights = DEFAULT_WEIGHTS) -> list[dict]:
    overall = (
...
| 외부 검색 | Naver Image Search API · Google Custom Search API · Openverse API |
