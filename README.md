...
- DesignProfile은 한국어 쿼리(100개 이상의 번역 맵 포함)를 포함한 자유 형식 입력을 안정적이고 비교 가능한 구조로 정규화합니다. 동일한 프로파일이 이미지 생성, 프로바이더 검색, 랭킹의 세 시스템을 모두 구동합니다. 정렬된 키워드와 dominant color의 SHA-256 해시가 프로파일 ID(`profile_hash`)를 결정하며, 의미적으로 동일한 쿼리의 중복 연산을 방지합니다.

- 랭킹 엔진은 단순 태그 매칭 대신 TF-IDF 코사인 유사도, RGB 색상 거리, 키워드 겹침, 메타데이터 품질 신호를 가중 합산하여 결과를 의미 있는 순서로 정렬합니다.
...
FMD는 디자이너나 기획자가 특정 디자인 스타일을 찾는 과정을 단순 키워드 검색이 아닌 의도 기반 검색으로 해결하는 것을 목표로 합니다.

예를 들어 사용자가 다음과 같이 디자인을 설명할 수 있습니다.

minimal black typography poster

검색 과정은 다음과 같이 진행됩니다.

사용자가 텍스트로 원하는 디자인 스타일을 입력합니다

시스템이 입력된 텍스트를 분석하여 DesignProfile을 생성합니다

생성된 DesignProfile을 기반으로 여러 디자인 플랫폼을 동시에 검색합니다

검색 결과를 랭킹 알고리즘으로 정렬합니다

가장 관련성이 높은 디자인 결과를 사용자에게 제공합니다

이 구조를 통해 사용자는 여러 디자인 플랫폼을 직접 탐색하지 않고도 원하는 디자인 스타일을 빠르게 찾을 수 있습니다.
---
### Extensibility

FMD는 다양한 디자인 플랫폼과 검색 알고리즘을 확장할 수 있도록 설계되었습니다.

- 새로운 디자인 플랫폼 추가

멀티 프로바이더 검색 구조를 사용하여 새로운 디자인 플랫폼을 쉽게 추가할 수 있습니다.
...
새로운 플랫폼은 Adapter 형태로 추가할 수 있으며 기존 검색 구조를 변경하지 않아도 됩니다.

- 검색 알고리즘 확장

검색 결과의 품질을 개선하기 위해 다양한 랭킹 알고리즘을 적용할 수 있습니다.

예를 들어 다음과 같은 요소를 추가할 수 있습니다.
...
DesignProfile 구조를 확장하면 새로운 검색 신호를 쉽게 추가할 수 있습니다.
...
layout
mood
새로운 속성을 추가해도 검색 시스템 전체 구조를 변경할 필요가 없습니다.
---
### Engineering Insight

FMD 프로젝트를 개발하면서 검색 시스템과 프론트엔드 구조 설계에 대해 몇 가지 중요한 점을 배울 수 있었습니다.

검색 시스템에서 데이터 구조 설계의 중요성

검색 기능을 구현할 때 단순 문자열 기반 검색보다 사용자 입력을 구조화된 데이터로 변환하는 방식이 훨씬 효과적이라는 것을 경험했습니다.
DesignProfile 구조를 도입함으로써 검색 조건을 명확하게 정의할 수 있었고 검색 알고리즘을 확장하기도 쉬워졌습니다.

프론트엔드는 데이터 흐름의 중심이 될 수 있다

검색 시스템에서는 프론트엔드가 단순히 결과를 표시하는 역할이 아니라 사용자 입력을 구조화하고 검색 요청을 구성하는 중요한 역할을 한다는 것을 배웠습니다.
프론트엔드에서 검색 조건을 어떻게 설계하느냐에 따라 전체 검색 경험이 크게 달라질 수 있습니다.

확장 가능한 구조 설계의 중요성

여러 디자인 플랫폼을 검색하는 기능을 구현하면서 새로운 데이터 소스를 쉽게 추가할 수 있는 구조가 중요하다는 것을 느꼈습니다.
멀티 프로바이더 검색 구조와 Adapter 패턴을 사용하면 새로운 플랫폼을 추가할 때 기존 시스템을 크게 수정하지 않아도 됩니다.
---
## 주요 기능
